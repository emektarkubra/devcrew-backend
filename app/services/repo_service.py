...
async def fetch_file_content(
    access_token: str, owner: str, repo: str, file_path: str
) -> str:
...
async def fetch_all_repo_files(
    ...
    if _root:
        async with httpx.AsyncClient(timeout=30.0) as client:
            return await fetch_all_repo_files(
                access_token, owner, repo, path, client, _root=False
            )
    ...
    for item in items:
        if item["type"] == "file":
            ...
            sub_files = await fetch_all_repo_files(
                access_token, owner, repo, item["path"], client, _root=False
            )
