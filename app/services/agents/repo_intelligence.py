...
async def get_repo_intelligence(
    ...
    bug_issues = [
        ...
        and any(
            l["name"].lower() in ["bug", "fix", "error"] for l in i.get("labels", [])
        )
    ]
    ...
    bugs_this_week = sum(
        ...
        for i in bug_issues
        if (
            datetime.utcnow() - datetime.fromisoformat(i["created_at"].replace("Z", ""))
        ).days
        <= 7
    )
    ...
    contributor_list = [
        {
            ...
            "prs": sum(
                1 for pr in prs if pr.get("user", {}).get("login") == c.get("login")
            ),
    ...
    modules = []
    for path, change_count in sorted(file_change_count.items(), key=lambda x: -x[1])[
        :8
    ]:
        ...
        modules.append(
            {
                ...
                "lastChanged": time_ago(last_commit["commit"]["author"]["date"])
                if last_commit
                else "",
