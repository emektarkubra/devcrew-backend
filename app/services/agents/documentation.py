...
def _is_main_router(f: str) -> bool:
    filename = f.split("/")[-1]
    return filename in IS_MAIN_ROUTER_NAMES or filename.lower() in {
        n.lower() for n in IS_MAIN_ROUTER_NAMES
    }
...
def _handle_ai_exception(e: Exception, owner: str, repo: str, target: str):
    ...
    if (
        ...
        raise AIRateLimitError(owner=owner, repo=repo, error_str=str(e)) from e

    if "request too large" in err_str or "context_length" in err_str:
        raise ValidationAppError(
            code="CONTEXT_TOO_LARGE",
            message="Request too large for AI model. Try a smaller file.",
...
async def generate_documentation(
    ...
    if is_repo_level:
        ...
        # ── API ──────────────────────────────────────────────────────
            ...
            schema_files = [
                f for f in file_list if _is_schema(f) and f.endswith(CODE_EXTENSIONS)
            ]
            all_routes = [
                f
                for f in file_list
        ...
        # ── README ───────────────────────────────────────────────────
            ...
            code_files = [
                f
                for f in file_list
                if f.endswith(CODE_EXTENSIONS) and f not in already_included
            ][:limit]
                ...
                if doc_type == "api":
                    if _is_main_router(f):
                        ...
                        route_contents.append(
                            ...
                            f"{_extract_route_signatures(content, f)[:1000]}"
                        )
                    else:
        ...
        all_contents = (
            schema_contents + router_contents + route_contents + other_contents
        )
        context = "\n\n".join(all_contents)
        if doc_type == "api":
            context = context[:10500]
        else:
        ...
        for chunk in chunks[:3]:
            try:
                ...
                all_answers.append(chunk_answer.strip())
            except Exception as e:
                _handle_ai_exception(e, owner, repo, target)
