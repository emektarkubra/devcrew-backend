...
from app.schemas.agents import (
    ...
    DocumentationHistoryRequest,
    RepoFilesRequest,
    SaveTestsRequest,
    TestGeneratorRequest,
    TestHistoryRequest,
    ApplyFixesToBranchRequest,
    ApplyDebugFixRequest,
    CheckIndexResponse,
    PRReviewResponse,
    PRHistoryItemResponse,
    DebugResponse,
    DebugHistoryItemResponse,
    DebugApplyFixResponse,
    TestGeneratorResponse,
    TestHistoryItemResponse,
    SaveTestsResponse,
    DocumentationResponse,
    DocumentationHistoryItemResponse,
    RepoFilesResponse,
    ApplyFixesResponse,
    ApplyFixesToBranchResponse,
    TokenRequest,
)
from app.services.agents.pr_review import (
    generate_fixes,
    pr_review,
...
async def test_generator_history(
    payload: TestHistoryRequest, db: Session = Depends(get_db)
):
    user_id = get_current_user_id(payload.token)
    repo_full = f"{payload.owner}/{payload.repo}"
...
@router.post(
    "/documentation/history", response_model=List[DocumentationHistoryItemResponse]
)
async def documentation_history(
    payload: DocumentationHistoryRequest, db: Session = Depends(get_db)
):
    user_id = get_current_user_id(payload.token)
    repo_full = f"{payload.owner}/{payload.repo}"
...
@router.post(
    "/pr-review/apply-fixes-to-branch", response_model=ApplyFixesToBranchResponse
)
async def apply_fixes_to_branch_endpoint(
    payload: ApplyFixesToBranchRequest, db: Session = Depends(get_db)
):
    user_id = get_current_user_id(payload.token)
    user = db.query(User).filter(User.id == user_id).first()
